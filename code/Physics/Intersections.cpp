...
bool RaySphare(const Vec3& rayStart, const Vec3& rayDir, const Vec3& sphereCenter, const float sphereRadius, float &t1, float &t2) {
  ...
  const Vec3& velA, const Vec3& velB, const float dt, Vec3& ptOnA, Vec3& ptOnB, float& toi)
{
  ...
  else if (!RaySphare(posA, rayDir, posB, shapeA->m_radius + shapeB->m_radius, t0, t1)) {
    return false;
  }
  ...
  // Get the earliest positive of impact
  toi = (t0 < 0.0f) ? 0.0f : t0;

  // If the erliest colllision is too far in the future, then there's no collision this frame
  if (toi > dt) {
	return false;
  }
...
bool Intersect( Body * bodyA, Body * bodyB, const float dt, contact_t & contact ) {
  ...
  if (bodyA->m_shape->GetType() == Shape::SHAPE_SPHERE && bodyB->m_shape->GetType() == Shape::SHAPE_SPHERE) {
	  ...
	  contact.ptOnA_WorldSpace, contact.ptOnB_WorldSpace, contact.timeOfImpact))
	{
	  ...
	  contact.ptOnA_LocalSpace = bodyA->WorlSpaceToBodySpace(contact.ptOnA_WorldSpace);
	  contact.ptOnB_LocalSpace = bodyB->WorlSpaceToBodySpace(contact.ptOnB_WorldSpace);
